...
`docker compose config --quiet` 只驗證設定，不輸出展開後的環境變數。需要確認服務名稱時可使用 `docker compose config --services`；不要把完整合併設定貼到 CI log、Issue 或螢幕分享。[Docker Compose config](https://docs.docker.com/reference/cli/docker/compose/config/)
...
Compose 中使用 `docker compose config --quiet` 驗證變數可解析；使用 `docker compose config --services` 確認服務名稱，避免輸出完整環境變數。
