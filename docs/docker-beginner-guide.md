...
| 想在本機管理 keys、users、teams 或 UI | 官方 Docker Compose | Proxy、Postgres、Prometheus、named volumes | 正式環境級備份、高可用與外部 secrets 管理 |
...
單一容器且未連接資料庫時，預期會看到 status 欄位，但 db 可能是 Not connected。連接 Postgres 後，官方範例會回報 status: connected、db: connected 及 LiteLLM 版本。[LiteLLM Health Checks：readiness](https://docs.litellm.ai/docs/proxy/health)
...
## 5. 官方 Docker Compose：Proxy、Postgres 與 Prometheus

LiteLLM 官方 Compose 範本會啟動 Proxy、Postgres 與 Prometheus。官方快速入門要求在 docker compose up 前準備 .env、config.yaml 與 prometheus.yml，並確認 Compose 檔案中的 config volume 與 --config 命令沒有被註解。[LiteLLM Docker quick start](https://docs.litellm.ai/docs/proxy/docker_quick_start)、[官方 docker-compose.yml](https://github.com/BerriAI/litellm/blob/main/docker-compose.yml)
