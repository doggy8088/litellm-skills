...
description: 操作、維護、稽核或修復 LiteLLM Proxy 部署。當任務涉及 Postgres logical backup／restore、Azure Blob 上傳或 SpendLogs 封存復原時使用；virtual key 管理、機密稽核、成本政策、觀測設定與安全規則仍搭配對應專門技能及官方管理工具。
---
...
依任務讀取 [備份與還原](references/backup-restore.md) 或 [SpendLogs 封存](references/spendlogs-archives.md)。Virtual key 管理改用 `litellm-cost-governance` 與已審查版本的官方 LiteLLM skills／API；機密盤點與清理改用 `litellm-guardrails-safety`。

## 安全底線

- 不輸出或提交 virtual key、key hash、admin token、SAS signature、密碼或 database URL。
- Key CSV、SpendLogs 匯出、備份與用量報告一律視為敏感資料；不得將 `.env` 放進備份。
- 備份與封存必須使用限制權限、明確 age recipient 的 authenticated encryption 及明確保留期限；private identity 只以受保護檔案路徑傳入。SAS URL 僅能使用 HTTPS，且不得寫入 log。
